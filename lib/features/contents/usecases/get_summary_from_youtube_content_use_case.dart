...
import 'package:techtalk/features/contents/repositories/entities/youtube_ai_summary_entity.dart';
...
class GetSummaryFromYoutubeContentUseCase extends BaseUseCase<
    YoutubeVideoAndCaptionEntity, Result<YoutubeAiSummaryEntity>> {
  @override
  FutureOr<Result<YoutubeAiSummaryEntity>> call(
...
1. **응답 유형 (`type`) 지정**:
   ...
   - `isValid` 타입이 아니면 `main_theme`과 `summaries`을 제공하지 않음.
...
```json 
{
  ...
  ```
    ...
    try {
      ...
      log('토큰사용량 : ${completion.usage}'); // 응답 결과 출력
      log('시간 : ${DateTime.now().difference(startTime).inSeconds}'); // 응답 결과 출력
      log('결과랑이 : ${completion.choices.first.message.content?.first.text}'); // 응답 결과 출력

      final response = completion.choices.first.message.content?.first.text;

      if (response == null) return Result.failure(const YtTimeoutException());

      final targetJson = jsonDecode(response);
      final targetEntity = YoutubeAiSummaryEntity.fromJson(targetJson);
      return Result.success(targetEntity);
    } on RequestFailedException catch (e) {
      log('GetSummaryFromYoutubeContentUseCase : $e');
      return Result.failure(const YtUnexceptedGptException());
    } catch (e) {
      if (e is TimeoutException) {
        log('GetSummaryFromYoutubeContentUseCase / TimeoutException / $e');
        return Result.failure(const YtTimeoutException());
      } else if (e is FormatException) {
        log('GetSummaryFromYoutubeContentUseCase / FormatException :$e');
        return Result.failure(const YtJsonFormatException());
      }
